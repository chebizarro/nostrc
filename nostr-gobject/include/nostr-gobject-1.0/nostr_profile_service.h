...
/**
 * GnostrRelayUrlProvider:
 * @out: (element-type utf8): a GPtrArray to append relay URL strings into
 *
 * Callback type for providing relay URLs to the profile service.
 * The app layer registers this so the library can auto-configure relays
 * without depending on app-specific config/GSettings code.
 */
typedef void (*GnostrRelayUrlProvider)(GPtrArray *out);

/**
 * gnostr_profile_service_set_relay_provider:
 * @provider: (nullable): callback to load relay URLs, or NULL to unset
 *
 * Registers an app-level callback that supplies relay URLs as a fallback
 * when no relays have been explicitly set via gnostr_profile_service_set_relays().
