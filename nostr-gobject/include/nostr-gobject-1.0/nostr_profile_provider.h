...
/**
 * GnostrFollowListProvider:
 * @pubkey_hex: 64-char hex pubkey of the user
 *
 * Callback type for providing follow-list pubkeys to the pre-warm system.
 * The app layer registers this so the library can request follow-list data
 * without depending on app-specific follow-list code.
 *
 ...
 */
typedef gchar** (*GnostrFollowListProvider)(const gchar *pubkey_hex);

/**
 * gnostr_profile_provider_set_follow_list_provider:
 * @provider: (nullable): callback to provide follow-list pubkeys, or NULL to unset
 *
 * Registers an app-level callback that supplies follow-list pubkeys for pre-warming.
 * Call this once during app init, e.g.:
...
/* Pre-warm the LRU cache from NDB for a user and their follow list.
 * Runs asynchronously in a GTask worker thread.
 * Call after login when user_pubkey_hex is known.
 * If a follow-list provider has been set, it will also pre-warm follow-list profiles. */
void gnostr_profile_provider_prewarm_async(const char *user_pubkey_hex);

/* Pre-warm the LRU cache for a batch of pubkeys on a background
