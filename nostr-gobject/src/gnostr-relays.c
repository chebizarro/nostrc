...
static GSettings *relays_settings_new(void) {
  if (!s_relays_schema_id) return NULL;
  GSettingsSchemaSource *src = g_settings_schema_source_get_default();
  if (!src) return NULL;
  GSettingsSchema *schema = g_settings_schema_source_lookup(src, s_relays_schema_id, TRUE);
  if (!schema) return NULL;
  GSettings *settings = g_settings_new(s_relays_schema_id);
...
void gnostr_save_relays_from(GPtrArray *list) {
  ...
  if (settings) {
    ...
    gboolean wrote_settings = g_settings_set_strv(settings, "relays", (const gchar * const*)strv);
...
void gnostr_save_dm_relays_from(GPtrArray *list) {
  ...
  if (settings) {
    g_auto(GStrv) strv = g_new0(gchar*, list->len + 1);
    for (guint i = 0; i < list->len; i++) strv[i] = g_strdup((const gchar*)list->pdata[i]);
    gboolean wrote_settings = g_settings_set_strv(settings, "dm-relays", (const gchar * const*)strv);
...
GSettings *gnostr_relay_get_settings(void) {
  ...
  s_relay_settings = relays_settings_new();
