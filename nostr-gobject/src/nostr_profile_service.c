...
/* Check the in-memory/provider-backed nostrdb cache for a profile.
 * gnostr_profile_provider_get() already falls through to the direct profile
 * FlatBuffer reader and populates its LRU. Do not repeat the lookup through
 * kind:0 event JSON here. */
static GnostrProfileMeta *check_ndb_cache(const char *pubkey_hex) {
  return gnostr_profile_provider_get(pubkey_hex);
}
