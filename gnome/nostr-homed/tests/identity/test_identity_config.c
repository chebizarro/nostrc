...
static void write_text(const char *path, const char *text) {
  ...
  NH_CHECK(file); NH_CHECK(fputs(text, file) >= 0); NH_CHECK(fclose(file) == 0);
}

int main(void) {
  char path[] = "/tmp/nostr-identity-config-XXXXXX";
  nh_identity_config config;
  int fd = mkstemp(path); NH_CHECK(fd >= 0); close(fd);
  write_text(path,
  ...
  NH_CHECK(nh_identity_config_load(path, &config) == NH_IDENTITY_OK);
  NH_CHECK(strcmp(config.authority_path, "/tmp/authority.db") == 0);
  write_text(path, "uid_min=200000\nuid_min=200001\n");
  NH_CHECK(nh_identity_config_load(path, &config) == NH_IDENTITY_INVALID);
  write_text(path, "unknown_security_option=yes\n");
  NH_CHECK(nh_identity_config_load(path, &config) == NH_IDENTITY_INVALID);
  nh_identity_config_defaults(&config);
  config.uid_min = 500000; config.uid_max = 550000;
  NH_CHECK(nh_identity_config_validate(&config) == NH_IDENTITY_INVALID);
  nh_identity_config_defaults(&config);
