...
static nh_auth_transaction *begin_login(mock_authority *mock,
  ...
  assert(nh_auth_transaction_begin(&authority, p, &request, &tx) ==
         NH_AUTH_TX_OK);
  return tx;
}
static nh_auth_receipt verify_login(nh_auth_transaction *tx,
  ...
  assert(nh_auth_transaction_select_provider(
         ...
         NH_AUTH_TX_OK);
  assert(nh_auth_transaction_begin_proof(tx, p, 1002, &challenge_deadline) ==
         NH_AUTH_TX_OK);
  assert(challenge_deadline == 121002);
  assert(nh_auth_transaction_start_verification(tx, p, 1003) == NH_AUTH_TX_OK);
  nh_auth_receipt receipt;
  assert(nh_auth_transaction_finish_verification(tx, p, 1004, NH_AUTH_PROOF_OK,
...
static void test_admission(void) {
  ...
  nh_auth_begin_request request = {NH_AUTH_PURPOSE_LINUX_LOGIN, "n_test",
                                   "gdm-password", 1000};
  nh_auth_transaction *tx = NULL;
  assert(nh_auth_transaction_begin(&authority, &root, &request, &tx) ==
         NH_AUTH_TX_OK);
  assert(strlen(nh_auth_transaction_get_id(tx)) == 64);
  assert(!strcmp(nh_auth_transaction_get_service(tx), "gdm-password"));
  assert(nh_auth_transaction_get_purpose(tx) == NH_AUTH_PURPOSE_LINUX_LOGIN);
  assert(nh_auth_transaction_get_account(tx)->uid == 200001);
  nh_auth_transaction_free(tx);

  nh_auth_peer_snapshot unprivileged = peer(1000, NH_AUTH_ENDPOINT_AUTH, 1);
  assert(nh_auth_transaction_begin(&authority, &unprivileged, &request, &tx) ==
         NH_AUTH_TX_UNAUTHORIZED);
  nh_auth_peer_snapshot wrong_endpoint = peer(0, NH_AUTH_ENDPOINT_USER, 1);
  assert(nh_auth_transaction_begin(&authority, &wrong_endpoint, &request,
  ...
  request = (nh_auth_begin_request){NH_AUTH_PURPOSE_SMB_CREDENTIAL, "ignored",
                                    "smb-credential", 1000};
  assert(nh_auth_transaction_begin(&authority, &own, &request, &tx) ==
         NH_AUTH_TX_OK);
  nh_auth_transaction_free(tx);
  own.uid = 200002;
  assert(nh_auth_transaction_begin(&authority, &own, &request, &tx) ==
...
static void test_binding_and_deadlines(void) {
  ...
  assert(nh_auth_transaction_select_provider(
             tx, &changed, NH_IDENTITY_PROVIDER_LOCAL_ENCRYPTED_KEY, 1001) ==
         NH_AUTH_TX_UNAUTHORIZED);
  assert(nh_auth_transaction_select_provider(tx, &owner,
  ...
  assert(nh_auth_transaction_select_provider(
         ...
         NH_AUTH_TX_DEADLINE);
  assert(nh_auth_transaction_get_state(tx) == NH_AUTH_TX_EXPIRED);
  nh_auth_transaction_free(tx);
  ...
  assert(nh_auth_transaction_select_provider(
         ...
         NH_AUTH_TX_OK);
  assert(nh_auth_transaction_begin_proof(tx, &owner, 1002, NULL) ==
         NH_AUTH_TX_OK);
  assert(nh_auth_transaction_start_verification(tx, &owner, 121003) ==
         NH_AUTH_TX_DEADLINE);
  nh_auth_transaction_free(tx);
}
...
static void test_one_winner_and_receipts(void) {
  ...
  bad.token[0] ^= 1;
  assert(nh_auth_transaction_open_receipt(tx, &owner, 1006, &bad) ==
         NH_AUTH_TX_UNAUTHORIZED);
  bad = receipt;
  ...
  assert(nh_auth_transaction_open_receipt(tx, &other_connection, 1006,
                                          &receipt) == NH_AUTH_TX_UNAUTHORIZED);
  assert(nh_auth_transaction_open_receipt(tx, &owner, 1006, &receipt) ==
         NH_AUTH_TX_OK);
  assert(mock.rechecks == 3);
  assert(nh_auth_transaction_open_receipt(tx, &owner, 1007, &receipt) ==
         NH_AUTH_TX_REPLAY);
  assert(nh_auth_transaction_close_session(tx, &owner) == NH_AUTH_TX_OK);
  nh_auth_transaction_free(tx);

  tx = begin_login(&mock, &owner);
  receipt = verify_login(tx, &owner);
  assert(nh_auth_transaction_open_receipt(tx, &owner,
...
static void test_failure_and_cancel(void) {
  ...
  assert(nh_auth_transaction_start_verification(tx, &owner, 1003) ==
         NH_AUTH_TX_STALE);
  assert(nh_auth_transaction_get_state(tx) == NH_AUTH_TX_DENIED);
  nh_auth_transaction_free(tx);
  ...
  assert(nh_auth_transaction_start_verification(tx, &owner, 1003) ==
         NH_AUTH_TX_OK);
  assert(nh_auth_transaction_finish_verification(tx, &owner, 1004,
  ...
  tx = begin_login(&mock, &owner);
  assert(nh_auth_transaction_cancel(tx, &owner) == NH_AUTH_TX_OK);
  assert(nh_auth_transaction_cancel(tx, &owner) == NH_AUTH_TX_OK);
  assert(nh_auth_transaction_select_provider(
             tx, &owner, NH_IDENTITY_PROVIDER_LOCAL_ENCRYPTED_KEY, 1001) ==
...
static void test_boundary_and_malformed_receipts(void) {
  ...
  assert(nh_auth_transaction_open_receipt(tx, &owner, 1005, &bad) ==
         NH_AUTH_TX_UNAUTHORIZED);
  bad = receipt;
