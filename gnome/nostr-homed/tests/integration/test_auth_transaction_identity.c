...
int main(void) {
  ...
  NH_CHECK(nh_auth_transaction_begin(&authority, &peer, &request, &tx) ==
  ...
  NH_CHECK(nh_identity_store_lookup_by_name(store, request.username, &account) ==
         NH_IDENTITY_OK);
  ...
  NH_CHECK(nh_auth_transaction_select_provider(
         ...
         NH_AUTH_TX_OK);
  NH_CHECK(nh_auth_transaction_begin_proof(tx, &peer, 1002, NULL) ==
         NH_AUTH_TX_OK);
  NH_CHECK(nh_auth_transaction_start_verification(tx, &peer, 1003) ==
         NH_AUTH_TX_OK);
