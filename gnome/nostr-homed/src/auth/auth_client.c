...
/* Sends one op with the given payload object and returns the response result. */
static int client_op(int fd, nh_auth_operation op, json_t *payload /*stolen*/,
  ...
  rc = -1;
  json_error_t e;
  json_t *root = response.payload_json ? json_loads(response.payload_json, 0, &e)
                                       : NULL;
...
int nh_auth_client_login(int fd, const char *username, const char *service,
  ...
  if (!username || !service || !passphrase || !result_out) return -1;
  ...
  if (client_op(fd, NH_AUTH_OP_BEGIN_LOGIN, begin, result_out) != 0) return -1;
  if (*result_out != NH_AUTH_RESULT_OK) return 0; /* denied/unknown/etc. */

  json_t *select = json_object();
  if (!select || json_object_set_new(select, "provider", json_string("local"))) {
    if (select) json_decref(select);
    return -1;
  }
  if (client_op(fd, NH_AUTH_OP_SELECT_PROVIDER, select, result_out) != 0) return -1;
  if (*result_out != NH_AUTH_RESULT_OK &&
      *result_out != NH_AUTH_RESULT_INTERACTION_REQUIRED)
    return 0;

  json_t *unlock = json_object();
  if (!unlock || json_object_set_new(unlock, "secret", json_string(passphrase))) {
