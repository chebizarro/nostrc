...
| ⚠️ 4 | Signer fuzz + binary NIP-44 tests | PARTIAL: `test_mock_signer_contract.c` shipped; libFuzzer harness + real-service `GTestDBus` contract test **deferred to nostrc-tf3b** (P2). Existing `test_sign_event_json` + `test_getrelays` cover D1.a contract. `nostrc-p7f6` + `nostrc-3m86` stay open. | `nips/nip55l/tests/`, `gnome/nostr-homed/tests/integration/test_mock_signer_contract.c` (new) | #1 | M |
...
| 13 | Session relay daemon + unit | `relay.sock` live per session | `$XDG_RUNTIME_DIR/nostr/relay.sock` 0600; SO_PEERCRED UID check; systemd `--user` unit | `apps/gnostr-signer/daemon/ipc.c:199-221,240-290` (pattern), new `gnome/nostr-homed/systemd/user/nostr-session-relay.service` | #12 | M |
| 14a | nostr-notify-daemon relay intake | NIP-29/DM subscriptions + coalesce + persistence | Filters live; dedup + cursor persistence; suppression generation guard passes | `libnostr/include/nostr-kinds.h:14-19,181-186`, new source under `gnome/nostr-homed/src/notify/` | #13 | L |
| 14b | nostr-notify-daemon shell UX | GNotification action opens GNostr thread | GNostr adds `nostr://open?…` deep-link URI handler; notifier's own `org.nostr.NotifyDaemon` GApplication ID; withdraw-on-focus | `apps/gnostr/src/notifications/desktop_notify.c:234-244,479-503` (GNostr side additions), new `gnome/nostr-homed/systemd/user/nostr-notify.service` | #14a | L |
...
| 16a | Samba server six-file config + service packaging | Six files land, install, `testparm -s` clean, unit enables cleanly on a stock host | 09-19 §1331-1336 files (enumerated §4.2); sysusers + timer wiring | #15 | L |
| 16b | Samba server installed-client acceptance | End-to-end file-serving demo against a real Windows/Explorer + GNOME Files client on the lab | Real client mounts, reads, writes, respects quotas, ACLs; retention across restart; passdb config drift-detector green | #16a, #18 | XL |
| 17 | Share-declaration v1 + VFS follow-up | Files served from materialized mount | `smb.conf.standalone.sample` points at porthome/FUSE mountpoint; VFS plugin bead filed | `docs/designs/home-from-relay.md` (FROZEN, consumer) | #16 | M |
| 18 | Journal↔passdb reconciliation | Dedicated config always selected | Adapter passes explicit `-s` config to `smbpasswd`/`pdbedit`; drift detector test | `gnome/nostr-homed/src/smb/passdb_tdbsam.c:205-277`, `nostr-authd.c:190-205` | #16 | M |
...
| 22a | Debian packaging | All new units + subpackages install and purge clean on Ubuntu 24.04 arm64 lab; lintian clean | `debian/*.install`, `debian/rules`, `debian/control` | #13, #14, #10, #16a | M |
| 22b | Fedora packaging | Same subpackage split in `packaging/rpm/nostr-login.spec`; rpmlint clean; `rpmbuild -bs` succeeds | `packaging/rpm/nostr-login.spec` | #22a | M |
