...
| ✅ 16b | Samba server installed-client acceptance | LANDED (Wave 4 `a7bea570`, RERUN `nostrc-nud0` 2026-09-26): Q5 matrix run against Windows 10 (`192.168.64.4`) + GNOME Files (`gio` under `dbus-run-session` on `192.168.64.3`). **Result: GREEN** post Wave-5 P1 fixes (`5db84f7a`). Original AMBER — W2/W3/W4 (134 MB/s + sha256 match)/W5/W7 (SMB 3.1.1 encrypted)/W8/W9 + G1/G2 all GREEN. W1 mount + W6 ACL escalation AMBER. Two P1 packaging defects (`nostrc-0vuu`, `nostrc-45b2`) FIXED in Wave 5 A (`5db84f7a`) and validated by rerun. P1 passdb drift-detector belongs to `nostr-authd` scope, not this matrix. Full transcripts + `tests/acceptance/samba-server-windows/` harness. `nostrc-rb0e`/`.11`/`.12` remain open until the two P1s land + re-run turns GREEN | `docs/reviews/samba-server-windows-acceptance-2026-09-26.md` (original) + `docs/reviews/samba-server-windows-acceptance-rerun-2026-09-26.md` (Wave-5 rerun, GREEN), `tests/acceptance/samba-server-windows/*` | #16a ✅, #18 ✅ | XL |
...
| ⚠️ 22c | Installed-unit acceptance | **Debian arm LANDED** (nostrc-qngf, docs/reviews/wave4-22c-debian-installed-acceptance-2026-09-26.md): 5 Wave-4 packages build + lintian clean + install + all boot + dep-purity green + purge clean, on aarch64 Ubuntu 24.04. Three inline P1/P2 defects fixed during the run: nostrc-sn0j (aarch64 -Werror), nostrc-8oon (user unit cap-drop), nostrc-il9p (missing libnip19). **Fedora arm nostrc-cr0s** remains blocked on a Fedora VM | Both #22a + #22b landed | #22a, #22b, #21 | M |
| ⚠️ 23 | Live smoke lab | **PARTIAL GREEN** on nostrc-0xd3 (docs/reviews/wave4-23-live-smoke-lab-2026-09-26.md). Shipping-code steps 1 (build clean), 3 (dep-purity), 4e-4h (stack-up + DAV protocol + Samba round-trip) all GREEN. Full interactive-login E2E blocked on FOUR pre-existing P2 enrollment defects surfaced by the run: nostrc-9yrr (seed→show gap), nostrc-u7su (first-boot reconcile refusal), nostrc-hfks (authd/smbd 0700 vs 0755), nostrc-y4ye (libnip34 missing). None are Wave-4 regressions. 0xd3 stays open pending those four fixes | bizarro@192.168.64.3 passes smoke matrix | All tracks | All | M |
