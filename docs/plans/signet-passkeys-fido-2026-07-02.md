...
Follow-ups: live-daemon D-Bus/NIP-46 end-to-end interop (`nostrc-8kc`, open); ~~replace spike CBOR encoder with libcbor + decode~~ **done** (`nostrc-h8a` — fido_cbor now on libcbor 0.14 with shared COSE/CTAP2 decode; python-fido2 interop still passes). Standalone meson include paths fixed (`libgo/include`/`nsync`/`nip11`).
