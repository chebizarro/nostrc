...
> Build tip for implementers: the full daemon builds in the monorepo — `cmake -S . -B build -DSIGNET_ENABLE_PASSKEYS=ON && cmake --build build --target signetd`. The standalone meson build now compiles all sources and runs the passkey tests (`meson setup bd signet -Dsignet_enable_passkeys=true -Dsignet_require_in_tree_libs=false && meson test -C bd`); `libgo/include` + `nsync` + `nip11` include paths are wired. (Standalone `signetd` *linking* still needs the monorepo lib graph — meson's in-tree lib names are stale vs CMake — so use the CMake `build/` to build/verify the daemon itself.)
