...
> Build tip for implementers: two working build paths. CMake monorepo — `cmake -S . -B build -DSIGNET_ENABLE_PASSKEYS=ON && cmake --build build --target signetd`. Standalone meson — after building the monorepo CMake libs once (they live under `../build`), `meson setup bd signet -Dsignet_enable_passkeys=true && meson compile -C bd && meson test -C bd` now fully builds `signetd`/`signetctl` and passes all 9 tests. The meson in-tree link line mirrors CMake's (nostrc-1fb).
...
- [x] **Phase 4** — virtual CTAP-HID device. Done: `fido_ctaphid.c` (CTAP-HID framing INIT/CBOR/PING/CANCEL/ERROR/KEEPALIVE, per-channel reassembly, size limits) + minimal CTAP2 adapter (GetInfo/MakeCredential/GetAssertion) over `SignetFidoService`; honest getInfo (`up`/`rk` true, `uv`/`clientPin` false); Linux `/dev/uhid` behind `#if __linux__`, clean stub off-Linux; `[passkeys] virtual_ctap=false` (requires `enabled`); daemon lifecycle wired. Verified on macOS: signetd links, `test_fido_ctaphid`/`fido_service`/`store_passkeys`/`capability`/`fido_interop_external` pass. **Functional /dev/uhid + libfido2/fido2-token validation requires a Linux host** (steps documented in INTEGRATION.md). (`nostrc-71a`)

Follow-ups: ~~live-daemon D-Bus/NIP-46 end-to-end interop~~ **done** (`nostrc-8kc` — `signet/tests/interop/run_live_dbus_interop.sh` drives live `signetd` over a private D-Bus session through the full ceremony incl. export/import; python-fido2 verifies. Test-only daemon hooks are compile-gated behind `SIGNET_ENABLE_TEST_HOOKS` (default OFF) so production `signetd` carries none of them); ~~replace spike CBOR encoder with libcbor + decode~~ **done** (`nostrc-h8a`). Standalone meson build fully fixed: include paths (`libgo/include`/`nsync`/`nip11`) **and** the full `signetd` link (`nostrc-1fb`, done — meson now links the monorepo archives + curl/nsync/secp256k1/websockets/flatccrt in CMake order; `meson compile` builds the daemon and all 9 tests pass).
