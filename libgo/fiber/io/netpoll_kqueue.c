...
int gof_netpoll_arm(int fd, int events, uint64_t deadline_ns) {
  (void)deadline_ns;
  if (kq == -1 && gof_netpoll_init() != 0)
    return -1;

  struct kevent kev[2];
  int n = 0;
  if (events & GOF_POLL_READ)
    EV_SET(&kev[n++], fd, EVFILT_READ, EV_ADD | EV_ENABLE | EV_CLEAR, 0, 0, NULL);
  if (events & GOF_POLL_WRITE)
...
#else
/* Fallback stubs when neither epoll nor IOCP applies (non-Apple, non-Linux, non-Windows). */
