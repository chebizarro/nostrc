...
int gof_netpoll_wait(int timeout_ms){ if(kq==-1) return -1; struct kevent kevs[64]; struct timespec ts; struct timespec *tsp = NULL; if(timeout_ms>=0){ ts.tv_sec = timeout_ms/1000; ts.tv_nsec = (timeout_ms%1000)*1000000; tsp = &ts; } int n = kevent(kq, NULL, 0, kevs, 64, tsp); for(int i = 0; i < n && ready_cb; ++i){ int events = 0; if(kevs[i].filter == EVFILT_READ) events |= GOF_POLL_READ; if(kevs[i].filter == EVFILT_WRITE) events |= GOF_POLL_WRITE; if(events) ready_cb((int)kevs[i].ident, events); } return n; }
