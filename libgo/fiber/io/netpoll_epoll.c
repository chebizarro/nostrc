...
int gof_netpoll_arm(int fd, int events, uint64_t deadline_ns) {
  (void)deadline_ns;
  if (epfd == -1 && gof_netpoll_init() != 0)
    return -1;

  struct epoll_event ev = {0};
  if (events & GOF_POLL_READ)  ev.events |= EPOLLIN;
  if (events & GOF_POLL_WRITE) ev.events |= EPOLLOUT;
...
void gof_netpoll_close(int fd) {
  if (epfd != -1) epoll_ctl(epfd, EPOLL_CTL_DEL, fd, NULL);
}
