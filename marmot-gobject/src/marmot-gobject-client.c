...
marmot_gobject_client_new(MarmotGobjectStorage *storage)
{
    ...
    /*
     * Create the underlying C Marmot instance.
     * Note: marmot_new() takes ownership of the storage, but since the
     * GObject storage owns it, we need the GObject to outlive the Marmot.
     * We keep a ref to ensure this.
     */
                           ...
                           gpointer task_data, GCancellable *cancellable)
{
    ...
    if (!hex_to_bytes(d->pubkey_hex, pubkey, 32) ||
        ...
        g_task_return_new_error(task, MARMOT_GOBJECT_ERROR, -1,
    ...
    if (d->relay_urls) {
        while (d->relay_urls[relay_count]) relay_count++;
    }
                     ...
                     gpointer task_data, GCancellable *cancellable)
{
    ...
    if (d->key_package_jsons) {
        while (d->key_package_jsons[kp_count]) kp_count++;
    }
    ...
    if (d->admin_pubkey_hexes) {
        while (d->admin_pubkey_hexes[admin_count]) admin_count++;
        if (admin_count > 0) {
            admin_pks = g_new0(uint8_t[32], admin_count);
            for (size_t i = 0; i < admin_count; i++) {
                if (!hex_to_bytes(d->admin_pubkey_hexes[i], admin_pks[i], 32)) {
                    g_free(admin_pks);
                    g_task_return_new_error(task, MARMOT_GOBJECT_ERROR, -1,
                                            "Invalid admin pubkey hex at index %zu", i);
                    return;
                }
            }
                     ...
                     gpointer task_data, GCancellable *cancellable)
{
    ...
    uint8_t *group_id_bytes = g_malloc(byte_len);

    if (!hex_to_bytes(d->mls_group_id_hex, group_id_bytes, byte_len)) {
        g_free(group_id_bytes);
        g_task_return_new_error(task, MARMOT_GOBJECT_ERROR, -1,
                                "Invalid MLS group ID hex");
        return;
    }

    MarmotGroupId gid = marmot_group_id_new(group_id_bytes, byte_len);
    g_free(group_id_bytes);
                                 ...
                                 GError **error)
{
    ...
    if (!hex_to_bytes(mls_group_id_hex, bytes, byte_len)) {
        g_free(bytes);
        g_set_error(error, MARMOT_GOBJECT_ERROR, -1, "Invalid hex");
        return NULL;
    }
