...
static void signetctl_usage(FILE *out) {
  fprintf(out,
    ...
    "  restore-existing <agent_id> (--sec -|--sec-from-store <db>)\n"
    "                 [--expected-pubkey <hex>]\n"
...
int main(int argc, char **argv) {
  ...
  if (strcmp(cmd, "provision") == 0) {
    ...
    if (restoring && adopt_sec && strcmp(adopt_sec, "-") != 0) {
      fprintf(stderr, "signetctl: restore-existing accepts secret input only from stdin\n");
      return 2;
    }
