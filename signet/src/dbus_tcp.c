...
static void signet_tcp_handle_method(GDBusConnection *connection,
  ...
  /* Transport-local auth: copy the authenticated agent id while holding the
   * connection-state mutex, then delegate shared Signer/Credentials dispatch. */
  char *agent_id = NULL;
  g_mutex_lock(&ds->mu);
  TcpConnState *state = (TcpConnState *)g_hash_table_lookup(
      ds->conn_states, connection);
  if (state && state->authenticated && state->agent_id)
    agent_id = g_strdup(state->agent_id);
  g_mutex_unlock(&ds->mu);
