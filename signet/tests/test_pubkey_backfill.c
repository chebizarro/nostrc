...
static void seed_legacy_row(const char *db_path, const char *agent_id,
  ...
  CHECK(st != NULL);
  CHECK(signet_store_put_agent(st, agent_id, sk_raw, 32, "legacy-secret", 1000) == 0);

  /* Confirm the row really is a legacy row (pubkey missing). */
  SignetAgentMeta meta;
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, agent_id, &meta) == 0);
  CHECK(meta.pubkey == NULL || meta.pubkey[0] == '\0');
...
static void seed_via_store(SignetStore *st, const char *agent_id,
                           const uint8_t sk_raw[32]) {
  CHECK(signet_store_put_agent(st, agent_id, sk_raw, 32, NULL, 1000) == 0);
}
...
static void test_backfill_and_collision_detection(void) {
  ...
  char out_pk[65] = {0};
  CHECK(signet_key_store_adopt_agent(ks, "modern-agent", modern_sk, modern_pk, NULL,
                                      BUNKER_PK, RELAYS, 1, out_pk, NULL) == SIGNET_ADOPT_OK);

  /* 1: before backfill the legacy pubkey is invisible to collision checks. */
  bool in_use = true;
  CHECK(signet_store_pubkey_in_use(st, legacy_pk, NULL, &in_use) == 0);
  CHECK(in_use == false);

  /* 2: backfill updates exactly the legacy row. */
  size_t updated = 0, failed = 0;
  CHECK(signet_key_store_backfill_pubkeys(ks, &updated, &failed) == 0);
  CHECK(updated == 1);
  CHECK(failed == 0);

  SignetAgentMeta meta;
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, "legacy-agent", &meta) == 0);
  CHECK(meta.pubkey != NULL);
  CHECK(g_ascii_strcasecmp(meta.pubkey, legacy_pk) == 0);
  signet_agent_meta_clear(&meta);
  ...
  CHECK(signet_store_pubkey_in_use(st, legacy_pk, NULL, &in_use) == 0);
  CHECK(in_use == true);
  CHECK(signet_store_pubkey_in_use(st, legacy_pk, "legacy-agent", &in_use) == 0);
  CHECK(in_use == false); /* excluded from its own check */

  /* 3: adopting the SAME custody key under a new agent_id is now rejected. */
  char out_pk2[65] = {0};
  CHECK(signet_key_store_adopt_agent(ks, "impostor", legacy_sk, legacy_pk, NULL,
  ...
  CHECK(signet_store_set_agent_pubkey(st, "legacy-agent", modern_pk) == 1);
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, "legacy-agent", &meta) == 0);
  CHECK(g_ascii_strcasecmp(meta.pubkey, legacy_pk) == 0); /* unchanged */
  signet_agent_meta_clear(&meta);
  CHECK(signet_store_set_agent_pubkey(st, "no-such-agent", legacy_pk) == 1);
  CHECK(signet_store_set_agent_pubkey(st, "legacy-agent", "not-a-pubkey") == -1);
  /* 64 chars but not hex must be rejected too. */
  CHECK(signet_store_set_agent_pubkey(st, "legacy-agent",
  ...
  /* Uppercase input is canonicalized to lowercase on write. */
  {
    ...
    CHECK(signet_store_set_agent_pubkey(st, "upper-agent", upper) == 0);
    SignetAgentMeta um;
    memset(&um, 0, sizeof(um));
    CHECK(signet_store_get_agent_meta(st, "upper-agent", &um) == 0);
    CHECK(um.pubkey && strcmp(um.pubkey, up_pk) == 0); /* stored lowercase */
    signet_agent_meta_clear(&um);
    bool up_in_use = false;
    CHECK(signet_store_pubkey_in_use(st, up_pk, NULL, &up_in_use) == 0);
    CHECK(up_in_use == true);
...
static void test_db_level_uniqueness(void) {
  ...
  CHECK(signet_store_put_agent_ex(st, "agent-a", sk1, 32, "secret-a", pk1,
                                   "provisioned", 1000) == 0);
  CHECK(signet_store_put_agent_ex(st, "agent-a", sk2, 32, "secret-a2", pk2,
                                   "rotated", 2000) == 0);
  SignetAgentMeta meta;
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, "agent-a", &meta) == 0);
  CHECK(meta.pubkey && g_ascii_strcasecmp(meta.pubkey, pk2) == 0);
  signet_agent_meta_clear(&meta);

  /* Binding a DIFFERENT agent to agent-a's pubkey fails with 1 and must NOT
   * delete agent-a (the old INSERT OR REPLACE behavior). */
  CHECK(signet_store_put_agent_ex(st, "impostor", sk3, 32, NULL, pk2,
                                   "adopted", 3000) == 1);
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, "agent-a", &meta) == 0); /* intact */
  CHECK(meta.pubkey && g_ascii_strcasecmp(meta.pubkey, pk2) == 0);
  signet_agent_meta_clear(&meta);
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, "impostor", &meta) != 0); /* absent */

  /* Reusing another agent's connect_secret also fails with 1, intact rows. */
  CHECK(signet_store_put_agent_ex(st, "agent-c", sk3, 32, "dup-secret", pk3,
                                   "provisioned", 4000) == 0);
  uint8_t sk4[32]; char pk4[65];
  gen_keypair(sk4, pk4);
  CHECK(signet_store_put_agent_ex(st, "agent-d", sk4, 32, "dup-secret", pk4,
                                   "provisioned", 5000) == 1);
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, "agent-c", &meta) == 0); /* intact */
  signet_agent_meta_clear(&meta);
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_get_agent_meta(st, "agent-d", &meta) != 0); /* absent */
   ...
   * another agent to the UPPERCASE form of an existing pubkey still fails. */
  {
    ...
    CHECK(signet_store_put_agent_ex(st, "impostor-uc", sk4, 32, NULL, upper_pk2,
                                     "adopted", 5500) == 1);
    memset(&meta, 0, sizeof(meta));
    CHECK(signet_store_get_agent_meta(st, "impostor-uc", &meta) != 0); /* absent */
  }

  /* Malformed pubkeys are rejected outright. */
  CHECK(signet_store_put_agent_ex(st, "bad-pk", sk4, 32, NULL, "deadbeef",
                                   "adopted", 5600) == -1);
  CHECK(signet_store_put_agent_ex(st, "bad-pk", sk4, 32, NULL,
  ...
  gen_keypair(sk5, pk5);
  gen_keypair(sk6, pk6);
  CHECK(signet_store_put_agent(st, "legacy-1", sk5, 32, NULL, 6000) == 0);
  CHECK(signet_store_put_agent(st, "legacy-2", sk6, 32, NULL, 7000) == 0);
...
static void test_backfill_duplicate_legacy_keys(void) {
  ...
  seed_legacy_row(db_path, "twin-a", sk);
  {
    ...
    SignetStore *st = signet_store_open(&scfg);
    CHECK(st != NULL);
    CHECK(signet_store_put_agent(st, "twin-b", sk, 32, NULL, 1100) == 0);
    signet_store_close(st);
  }
  ...
  CHECK(signet_store_get_agent_meta(st, "twin-a", &ma) == 0);
  CHECK(signet_store_get_agent_meta(st, "twin-b", &mb) == 0);
