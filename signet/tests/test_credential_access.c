...
static void add_agent(SignetStore *store, const char *agent_id,
  ...
  CHECK(signet_store_put_agent_ex(store, agent_id, sk, sizeof(sk),
...
static void add_policy(SignetPolicyRegistry *pr, const char *name,
  ...
  CHECK(signet_policy_registry_add(pr, &pol) == 0);
}
...
static gpointer append_thread(gpointer data) {
  ...
  for (int i = 0; i < arg->rounds; i++) {
    ...
    CHECK(signet_audit_log_append(arg->store, 2000000000 + i,
...
int main(void) {
  ...
  CHECK(signet_policy_registry_assign(policy, "agent-a", "token-access") == 0);
  CHECK(signet_policy_registry_assign(policy, "agent-b", "token-access") == 0);
  CHECK(signet_policy_registry_assign(policy, "agent-c", "no-token-cap") == 0);
  CHECK(signet_policy_registry_assign(policy, "agent-e", "limited") == 0);
  ...
  CHECK(signet_store_create_secret(store, "agent-a",
  ...
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_create_secret(store, "agent-e",
  ...
  CHECK(grant.lease_expires_at == now + 600);
  ...
  CHECK(acquire(&ctx, "agent-b", cred_a, SIGNET_CAP_CREDENTIAL_GET_TOKEN,
                 NULL, false, now, &grant) == SIGNET_CRED_ACCESS_NOT_OWNER);
  expected_entries++;
  CHECK(grant.record.payload == NULL);

  /* 4. A foreign lease cannot be burned by another agent. */
  CHECK(signet_store_issue_lease(store, "foreignlease00000000000000000001",
  ...
  CHECK(signet_store_consume_lease(store,
  ...
  CHECK(signet_policy_registry_assign(policy, "agent-a",
                                       "deny-api-token") == 0);
  CHECK(acquire(&ctx, "agent-a", cred_a, SIGNET_CAP_CREDENTIAL_GET_TOKEN,
  ...
  CHECK(signet_deny_list_add(deny, PUBKEY_A, "agent-a", "test", now) == 0);
  CHECK(acquire(&ctx, "agent-a", cred_a, SIGNET_CAP_CREDENTIAL_GET_TOKEN,
                 NULL, false, now, &grant) == SIGNET_CRED_ACCESS_DENY_LISTED);
  expected_entries++;
  CHECK(signet_deny_list_remove(deny, PUBKEY_A) == 0);
  ...
  memset(&meta, 0, sizeof(meta));
  CHECK(signet_store_revoke_secret(store, cred_a, now, &meta) ==
         SIGNET_SECRET_OK);
  signet_secret_metadata_clear(&meta);
  CHECK(acquire(&ctx, "agent-a", cred_a, SIGNET_CAP_CREDENTIAL_GET_TOKEN,
  ...
  CHECK(acquire(&no_policy_ctx, "agent-a", cred_a,
                 SIGNET_CAP_CREDENTIAL_GET_TOKEN, NULL, false, now,
                 &grant) == SIGNET_CRED_ACCESS_NO_CAPABILITY);
  expected_entries++;

  /* 13. Missing explicit capability in the request: error, still audited. */
  CHECK(acquire(&ctx, "agent-a", cred_a, NULL, NULL, false, now, &grant) ==
         SIGNET_CRED_ACCESS_ERROR);
  expected_entries++;

  /* 14. Rate limiting (limit 1/hour => burst 1). */
  CHECK(acquire(&ctx, "agent-e", cred_e, SIGNET_CAP_CREDENTIAL_GET_TOKEN,
  ...
  CHECK(signet_audit_log_count(store) == expected_entries);

  /* Chain is intact and redacted: the payload canary never reached it. */
  int64_t broken_id = 0;
  CHECK(signet_audit_verify_chain(store, 0, 0, &broken_id) == 0);
  CHECK(audit_rows_containing(store, PAYLOAD_CANARY) == 0);
  CHECK(audit_rows_containing(store, "superSecret") == 0);
  /* Sanity: the probe finds strings that ARE present. */
  CHECK(audit_rows_containing(store, "credential_access") > 0);
  CHECK(audit_rows_containing(store, "\"decision\":\"deny\"") > 0);
  CHECK(audit_rows_containing(store, "\"reason\":\"deny_listed\"") > 0);
  CHECK(audit_rows_containing(store, "\"reason\":\"not_owner\"") > 0);
  ...
  CHECK(signet_audit_log_count(store) ==
