...
static void populate(SignetStore *store,
  ...
  CHECK(signet_store_put_agent_ex(store, "owner", sk, sizeof(sk),
  ...
  CHECK(signet_store_secret_history_count(store, alpha_id) == 1);
  ...
  signet_secret_metadata_clear(&md);
  CHECK(signet_store_revoke_secret(store, beta_id, NOW + 2, NULL) ==
         SIGNET_SECRET_OK);

  CHECK(signet_audit_log_append(store, NOW, "owner", "credential_create",
                                 alpha_id, "test", NULL) == 0);
  CHECK(signet_audit_log_append(store, NOW + 1, "owner", "credential_rotate",
                                 alpha_id, "test", NULL) == 0);
}
...
static void verify_populated(SignetStore *store,
  ...
  CHECK(signet_store_get_secret_at(store, alpha_id, NOW + 3, &rec) ==
         SIGNET_SECRET_OK);
  CHECK(rec.payload_len == strlen("alpha-token-v2"));
  CHECK(memcmp(rec.payload, "alpha-token-v2", rec.payload_len) == 0);
  ...
  SignetSecretMetadata md;
  memset(&md, 0, sizeof(md));
  CHECK(signet_store_get_secret_metadata(store, alpha_id, NOW + 3, &md) ==
         SIGNET_SECRET_OK);
  CHECK(md.version == 2 && md.active_version == 2);
  CHECK(strcmp(md.provenance, "created") == 0);
  signet_secret_metadata_clear(&md);

  /* Revocation state survived. */
  CHECK(signet_store_get_secret_at(store, beta_id, NOW + 3, &rec) ==
         SIGNET_SECRET_REVOKED);
  CHECK(signet_store_get_secret_metadata(store, beta_id, NOW + 3, &md) ==
         SIGNET_SECRET_OK);
  CHECK(md.status == SIGNET_SECRET_STATUS_REVOKED);
  CHECK(strcmp(md.provenance, "imported") == 0);
  signet_secret_metadata_clear(&md);
...
static void test_backup_restore_roundtrip(void) {
  ...
  SignetStore *store = open_store(db, MASTER_KEY);
  CHECK(store != NULL);
  CHECK(signet_store_is_encrypted(store));
  char alpha_id[70], beta_id[70];
  populate(store, alpha_id, beta_id);

  CHECK(signet_store_backup(store, bak, BACKUP_KEY) == 0);
  CHECK(g_file_test(bak, G_FILE_TEST_EXISTS));
  /* The backup must be encrypted at rest and must carry 0600 permissions. */
  CHECK(!signet_store_file_is_plaintext_sqlite(bak));
  {
    GStatBuf st;
    CHECK(g_lstat(bak, &st) == 0);
  ...
  CHECK(signet_store_restore_backup(bak, BACKUP_KEY, db2, MASTER_KEY) == 0);
  ...
  CHECK(signet_store_restore_backup(bak, BACKUP_KEY, db, MASTER_KEY) == 0);
  char *pre = g_strdup_printf("%s.pre-restore", db);
  CHECK(g_file_test(pre, G_FILE_TEST_EXISTS));
  store = open_store(db, MASTER_KEY);
  CHECK(store != NULL);
...
static void test_wrong_keys_fail_closed(void) {
  ...
  CHECK(signet_store_restore_backup(bak, WRONG_KEY, db2, MASTER_KEY) == -1);
  CHECK(!g_file_test(db2, G_FILE_TEST_EXISTS));
  ...
  CHECK(signet_store_restore_backup(bak, BACKUP_KEY, db2, WRONG_KEY) == -1);
  CHECK(!g_file_test(db2, G_FILE_TEST_EXISTS));
  ...
  CHECK(signet_store_restore_backup(bak, BACKUP_KEY, bak, MASTER_KEY) == -1);
  CHECK(g_file_test(bak, G_FILE_TEST_EXISTS));

  /* Opening the encrypted store with the wrong master key must fail. */
  CHECK(open_store(db, WRONG_KEY) == NULL);
  /* Opening the backup with the master key (not the backup key) must fail. */
  CHECK(open_store(bak, MASTER_KEY) == NULL);
...
static void test_backup_refusals(void) {
  ...
  CHECK(signet_store_backup(store, bak, "short-passphrase") == -1);
  CHECK(!g_file_test(bak, G_FILE_TEST_EXISTS));

  /* Existing target is never overwritten. */
  FILE *fp = fopen(bak, "wb");
  CHECK(fp && fputs("sentinel", fp) >= 0);
  fclose(fp);
  CHECK(signet_store_backup(store, bak, BACKUP_KEY) == -1);
  gchar *contents = NULL;
  CHECK(g_file_get_contents(bak, &contents, NULL, NULL));
...
static void test_file_tamper_rejected(void) {
  ...
  CHECK(open_store(db, MASTER_KEY) == NULL);
