...
static int run_sign_without_connect_should_deny(void){
    ...
    /* No connect() sent; attempt sign_event directly */
    ...
    char *req_json = nostr_nip46_request_build("d1", "sign_event", params, 1);
    free(ev_json);
    ...
    int rc=0;
    if (!resp.id || !string_eq(resp.id, "d1")) { printf("id mismatch\n"); rc=11; }
    if (!resp.error) { printf("expected error for sign without connect\n"); rc=12; }
...
static int run_sign_after_connect_without_permission_should_deny(void){
    ...
    /* Now try to sign_event which should be denied */
    NostrEvent *ev = nostr_event_new();
    ev->kind = 1; ev->created_at = 123; ev->content = strdup("hello");
    char *ev_json = nostr_event_serialize(ev);
    nostr_event_free(ev);
    if (!ev_json){ printf("serialize unsigned event fail\n"); nostr_nip46_session_free(cli); nostr_nip46_session_free(bun); return 10; }

    const char *params2[1] = { ev_json };
    char *req_json = nostr_nip46_request_build("d2", "sign_event", params2, 1);
    free(ev_json);
    if (!req_json){ printf("req build fail\n"); nostr_nip46_session_free(cli); nostr_nip46_session_free(bun); return 11; }

    char *cipher_req=NULL; if (nostr_nip46_client_nip04_encrypt(cli, bunker_pk_sec1, req_json, &cipher_req) != 0 || !cipher_req){
        printf("encrypt fail\n"); free(req_json); nostr_nip46_session_free(cli); nostr_nip46_session_free(bun); return 12;
    }
    free(req_json);

    char *cipher_reply=NULL; if (nostr_nip46_bunker_handle_cipher(bun, client_pk_sec1, cipher_req, &cipher_reply) != 0 || !cipher_reply){
        printf("bunker handle fail\n"); free(cipher_req); nostr_nip46_session_free(cli); nostr_nip46_session_free(bun); return 13;
    }
    free(cipher_req);

    char *plain=NULL; if (nostr_nip46_client_nip04_decrypt(cli, bunker_pk_sec1, cipher_reply, &plain) != 0 || !plain){
        printf("decrypt fail\n"); free(cipher_reply); nostr_nip46_session_free(cli); nostr_nip46_session_free(bun); return 14;
    }
    free(cipher_reply);

    NostrNip46Response resp={0}; if (nostr_nip46_response_parse(plain, &resp) != 0){ printf("resp parse fail: %s\n", plain); free(plain); nostr_nip46_session_free(cli); nostr_nip46_session_free(bun); return 15; }
    free(plain);

    int rc=0;
    if (!resp.id || !string_eq(resp.id, "d2")) { printf("id mismatch\n"); rc=16; }
    if (!resp.error) { printf("expected error for sign without permission\n"); rc=17; }
...
int main(void){
    int rc = run_sign_without_connect_should_deny();
    if (rc){ printf("deny test1 failed rc=%d\n", rc); return rc; }
    rc = run_sign_after_connect_without_permission_should_deny();
    if (rc){ printf("deny test2 failed rc=%d\n", rc); return rc; }
    printf("test_bunker_sign_event_acl_deny: OK\n");
    return 0;
}
