...
int main(void) {
  ...
  /* Extract the actual event ID from the built request */
  char *actual_req_id = NULL;
  rc = nostr_json_get_string(req_json, "id", &actual_req_id);
  assert(rc == 0 && actual_req_id);
  ...
  /* Wallet builds a response using the actual request event ID */
  NostrNwcResponseBody resp = { .result_type = (char*)"get_balance", .result_json = (char*)"{\"balance\":42}" };
  char *resp_json = NULL;
  rc = nostr_nwc_wallet_build_response(&ws, actual_req_id, &resp, &resp_json);
  ...
  assert(out_req_id && strcmp(out_req_id, actual_req_id) == 0);
