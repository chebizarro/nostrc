...
int nostr_nwc_wallet_encrypt(const NostrNwcWalletSession *s,
  ...
  *out_ciphertext = NULL;
  if (s->enc == NOSTR_NWC_ENC_NIP44_V2) {
    ...
    if (parse_peer_xonly32(client_pub_hex, pkx) != 0) return -1;
    char *b64 = NULL;
    ...
    *out_ciphertext = b64; return 0;
    ...
    if (nostr_nip04_encrypt(plaintext, peer, wallet_sk_hex, &cipher, &err) != 0 || !cipher) {
      if (err) { free(err); err=NULL; }
      if (peer==sec1 && build_sec1_from_xonly(client_pub_hex, sec1, 0x03)==0) {
        if (nostr_nip04_encrypt(plaintext, sec1, wallet_sk_hex, &cipher, &err) != 0 || !cipher) { if (err) free(err); return -1; }
      } else {
        return -1;
      }
    }
  ...
  *out_plaintext = NULL;
  if (s->enc == NOSTR_NWC_ENC_NIP44_V2) {
    ...
    *out_plaintext = out; return 0;
    ...
    if (strlen(client_pub_hex)==64) { if (build_sec1_from_xonly(client_pub_hex, sec1, 0x02)==0) peer = sec1; }
    if (nostr_nip04_decrypt(ciphertext, peer, wallet_sk_hex, &plain, &err) != 0 || !plain) {
      if (err) { free(err); err=NULL; }
      if (peer==sec1 && build_sec1_from_xonly(client_pub_hex, sec1, 0x03)==0) {
        if (nostr_nip04_decrypt(ciphertext, sec1, wallet_sk_hex, &plain, &err) != 0 || !plain) { if (err) free(err); return -1; }
      } else {
        return -1;
      }
    }
