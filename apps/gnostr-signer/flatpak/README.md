...
```bash
flatpak-builder --verbose --force-clean build-dir org.gnostr.Signer.yaml
```
...
| File | Description |
|------|-------------|
| `org.gnostr.Signer.yaml` | Main Flatpak manifest (YAML format) |
| `org.gnostr.Signer.json` | Flatpak manifest (JSON format, equivalent) |
...
flatpak-builder --user --install build-dir org.gnostr.Signer.yaml
...
```bash
flatpak-builder --show-manifest org.gnostr.Signer.yaml
```
