...
guint nostr_gtk_timeline_tabs_add_tab(NostrGtkTimelineTabs *self,
  ...
  /* Close button (if closable) - use GtkGestureClick to capture clicks
   * before they propagate to the parent tab button (GTK4 button-in-button issue) */
  if (info->closable) {
    ...
    gtk_widget_set_focus_on_click(info->close_button, FALSE);

    /* Use gesture click at capture phase to handle clicks before parent button */
...
void nostr_gtk_timeline_tabs_remove_tab(NostrGtkTimelineTabs *self, guint index) {
  ...
  gboolean was_selected = (self->selected == index);
  ...
  /* If the closed tab was selected, emit tab-selected for the new selection
   * so listeners can update their state (e.g., model query) */
  if (was_selected && self->tabs->len > 0) {
    g_signal_emit(self, signals[SIGNAL_TAB_SELECTED], 0, self->selected);
  }
}
...
/* Gesture handler for close button - captures clicks at CAPTURE phase
 * to prevent them from propagating to the parent tab button */
static void on_close_gesture_pressed(GtkGestureClick *gesture, int n_press, double x, double y, gpointer user_data) {
  ...
  for (guint i = 0; i < self->tabs->len; i++) {
    ...
    if (info->close_button == close_btn) {
      /* Claim the gesture sequence to prevent parent button from handling */
