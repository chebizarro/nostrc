                          ...
                          GnostrPluginContext *context)
{
  ...
  /* NOTE: Do NOT auto-fetch from relays during activation.
   * nostrc-NEW: This caused ASAN crashes because the relay connection
   * infrastructure isn't ready during plugin activation (which happens
   * during app startup before the main loop runs). The crash occurred in
   * libwebsockets/OpenSSL TLS initialization.
   * Users can manually refresh via the settings page or the refresh action. */
